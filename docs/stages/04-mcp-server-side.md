...
### ⚠️ 后续：这个结论在阶段 F 被证据推翻了

阶段 F 建好评测集后，用同一套用例跑了对照实验，并且加了一个
**不同模型**的 LLM 裁判来评文案质量：

| | 保留推理 | 关推理 |
|---|---|---|
| 通过率 | 100% | 100% |
| 延迟 p50 | 8,030 ms | **2,788 ms**（2.9 倍） |
| 平均成本 | $0.002495 | **$0.000608**（4.1 倍） |
| 裁判均分 | {3.5, 3, 4} | {4, 4, 3.5} |

**裁判认为关推理的版本在 3 条里 2 条更高** —— 与上面"人工阅读"的判断相反。

于是默认值改成了**不保留推理**。理由：
**质量差异在本样本量下不可测，而性能与成本收益是确定的。
证据对质量不确定、对成本确定时，选成本低的一侧，保留回退开关。**

完整的实验设计与诚实说明见 [06-eval.md](06-eval.md)。

> 这一段的真正价值不在于"选对了哪个"，而在于：
> **先做了一个有依据的判断，然后建了一套能检验它的机制，最后按证据改了自己的结论。**
