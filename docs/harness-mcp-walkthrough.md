...
> 这是**真实存在过的 bug**，不是假想的。它不会让任何测试变红，
...
| "为什么不做 A/B 实验？" | **诚实说：没有真实流量，A/B 什么都测不出** —— 用自定义的代理指标去测写死的 mock 数据是"演戏"不是实验。项目做前后对比用的是 `python/eval/` 的评测集（12 条 golden set + 确定性断言 + `--baseline`）；早期那个从没被接线的 A/B 引擎已删除 |
